            ...
            <div className="relative z-10 container mx-auto pt-24 pb-20 px-6 max-w-6xl">
                ...
                <div className="mb-16 border-b border-zinc-200 pb-8 flex flex-col md:flex-row justify-between items-end gap-6">
                    <div className="space-y-2">
                        <span className="text-xs font-mono uppercase tracking-widest text-zinc-400">Identity</span>
                        <ProfileHeader userEmail={user?.email} className="text-4xl md:text-5xl font-light tracking-tight text-zinc-900" />
                    </div>
                    <div className="text-right hidden md:block">
                        <div className="text-xs font-mono text-zinc-400 mb-1">{new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</div>
                            ...
                            <div className="relative z-10 flex flex-col h-full justify-between gap-8">
                                <div className="space-y-4">
                                    ...
                                    <h2 className="text-3xl font-light max-w-lg leading-tight">
                                        {isVerifiedVendor
                                            ? <span>Manage <span className="italic font-serif text-zinc-400">{vendorData?.businessName}</span> inventory & analytics.</span>
                                            : vendorData 
                                                ? `Application is currently ${vendorData.verificationStatus}.`
                                                : <span>Turn your artifacts into <br/><span className="italic font-serif text-zinc-400">digital assets</span>.</span>}
                                    </h2>
                                </div>
                        ...
                        <section>
                            <div className="flex items-baseline justify-between mb-6">
                                <h3 className="text-xl font-light flex items-center gap-2">
                                    Acquisitions
                                    <span className="text-xs align-top bg-zinc-100 text-zinc-500 px-2 py-0.5 rounded-full font-mono">{recentOrders.length}</span>
                                </h3>
                                <Link href="/orders" className="text-xs font-medium text-zinc-500 hover:text-zinc-900 flex items-center gap-1 transition-colors">
                            ...
                            <div className="bg-white border border-zinc-100 rounded-xl overflow-hidden shadow-sm">
                                {recentOrders.length > 0 ? (
                                    recentOrders.map((order, i) => (
                                        <div key={order.id} className="group flex flex-col sm:flex-row sm:items-center justify-between p-5 border-b border-zinc-50 last:border-0 hover:bg-zinc-50/50 transition-colors cursor-pointer">
                                            <div className="flex items-start gap-4">
                                                ...
                                                <div>
                                                    <div className="font-medium text-zinc-800 group-hover:translate-x-1 transition-transform duration-300">{order.item}</div>
                    ...
                    <div className="lg:col-span-4 space-y-8">
                        ...
                        {user && (
                            <div className="bg-white rounded-2xl border border-zinc-100 shadow-sm p-2">
                                <div className="px-6 py-4 border-b border-zinc-50">
                                    <h3 className="text-sm font-semibold uppercase tracking-widest text-zinc-400">Preferences</h3>
                                </div>
                                <div className="p-4">
                        ...
                        <div className="pt-8 border-t border-zinc-200">
                            ...
                            <div className="flex flex-wrap gap-x-6 gap-y-2 text-xs font-medium text-zinc-400">
                                <span className="cursor-pointer hover:text-zinc-900 transition-colors">Agreements</span>
                                <span className="cursor-pointer hover:text-zinc-900 transition-colors">Privacy</span>
                                <span className="cursor-pointer hover:text-zinc-900 transition-colors">Support</span>
                            </div>
                        </div>
