...
export async function getAdminStats() {
    ...
    try {
        ...
        const [pendingVendorCount] = await db.select({ value: count() }).from(vendors).where(eq(vendors.verificationStatus, 'pending'));

        // MVP: Just summing up price text assuming it's parsable, or count purchases
        // In real app, price should be numeric.
        const [purchaseCount] = await db.select({ value: count() }).from(purchases).where(eq(purchases.status, 'completed'));
