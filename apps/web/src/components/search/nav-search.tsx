...
export function NavSearch() {
  ...
  return (
    <div ref={wrapperRef} className="relative w-full max-w-md z-50">
      ...
      >
        <Search
          className={`w-4 h-4 mr-3 transition-colors duration-300 ${isFocused ? 'text-primary' : 'text-stxt'}`}
        />
      ...
      <AnimatePresence>
        {isFocused && (query.length >= 2 || results.length > 0) && (
          <motion.div
            ...
            className="absolute top-full mt-3 left-0 w-full bg-card/90 backdrop-blur-2xl rounded-[1.5rem] shadow-2xl shadow-black/5 border border-stxt overflow-hidden py-2"
          >
            {results.length > 0 ? (
                ...
                {results.map((result, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <h4 className="font-serif font-bold text-sm text-txt group-hover:text-primary transition-colors">
                        {result.name}
                      </h4>
                      <p className="text-[10px] uppercase font-bold text-stxt tracking-wider">
                        {result.type}
                      </p>
                    </div>
